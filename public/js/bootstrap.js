...
+function ($) {
  ...
  $.fn.emulateTransitionEnd = function (duration) {
    ...
    $(
      ...
      $.support.transition.end, function () { called = true }
    )
...
+function ($) {
  ...
  $(
    ...
    'click.bs.alert.data-api', dismiss, Alert.prototype.close
  )
...
+function ($) {
  ...
  var Carousel = function (element, options) {
    ...
    this.options.pause == 'hover' && this.$element.on(
        'mouseenter',
        $.proxy(
          this.pause, this
        )
  ...
  Carousel.prototype.slide = function (type, next) {
    ...
    var e = $.Event(
      'slide.bs.carousel', { relatedTarget: $next[0], direction: direction }
    )
...
+function ($) {
  ...
  $(
    document
      ...
      'click.bs.dropdown.data-api', clearMenus
    ).on(
    ...
    function (e) {
      ...
      'click.bs.dropdown.data-api', toggle, Dropdown.prototype.toggle
    ).on(
      'keydown.bs.dropdown.data-api', toggle + ', [role=menu]', Dropdown.prototype.keydown
    )
...
+function ($) {
  ...
  Modal.prototype.show = function (_relatedTarget) {
    var that = this
    var e = $.Event(
      'show.bs.modal', { relatedTarget: _relatedTarget }
    )
    ...
    this.$element.on(
      'click.dismiss.modal', '[data-dismiss="modal"]',
      $.proxy(
        this.hide, this
      )
    )
    ...
    this.backdrop(
      function () {
        ...
        var e = $.Event(
          'shown.bs.modal', { relatedTarget: _relatedTarget }
        )

        transition ? that.$element.find(
            '.modal-dialog'
  ...
  Modal.prototype.backdrop = function (callback) {
    var that = this
    var animate = this.$element.hasClass(
      'fade'
    ) ? 'fade' : ''
...
+function ($) {
  ...
  Tooltip.prototype.init = function (type, element, options) {
    ...
    for (var i = triggers.length; i--;) {
      ...
      if (trigger == 'click') {
        ...
        this.$element.on(
          eventIn + '.' + this.type, this.options.selector,
          $.proxy(
            this.enter, this
          )
        )
        this.$element.on(
          eventOut + '.' + this.type, this.options.selector,
          $.proxy(
            this.leave, this
          )
        )
  ...
  Tooltip.prototype.show = function () {
    ...
    if (this.hasContent(
      ...
      var placement = typeof this.options.placement == 'function' ? this.options.placement.call(
        this, $tip[0], this.$element[0]
      ) : this.options.placement

      var autoToken = /\s?auto?\s?/i
      var autoPlace = autoToken.test(
        placement
      )
  ...
  Tooltip.prototype.applyPlacement = function (offset, placement) {
    ...
    if (/bottom|top/.test(
      ...
      this.replaceArrow(
        delta - width + actualWidth, actualWidth, 'left'
      )
    }
    else {
      this.replaceArrow(
        actualHeight - height, actualHeight, 'top'
      )
    }
...
+function ($) {
  ...
  Popover.prototype.getContent = function () {
    ...
    return $e.attr(
      'data-content'
    ) || (typeof o.content == 'function' ? o.content.call(
      $e[0]
    ) : o.content)
  }
...
+function ($) {
  ...
  ScrollSpy.prototype.process = function () {
    ...
    for (i = offsets.length; i--;) {
      activeTarget != targets[i] && scrollTop >= offsets[i] && (!offsets[i + 1] || scrollTop <= offsets[i + 1]) && this.activate(
        targets[i]
      )
    }
  }
...
+function ($) {
  ...
  Affix.prototype.checkPosition = function () {
    ...
    var affix = this.unpin != null && (scrollTop + this.unpin <= position.top) ? false : offsetBottom != null && (position.top + this.$element.height(
    ) >= scrollHeight - offsetBottom) ? 'bottom' : offsetTop != null && (scrollTop <= offsetTop) ? 'top' : false

    if (this.affixed === affix) return
    if (this.unpin) this.$element.css(
      'top', ''
    )
